...
O delineamento quadrado latino considera o controle local em dois sentidos e, portanto, assim como o delineamento em blocos, considera os três princípios básicos da experimentação: a repetição, a casualização e o controle local.

Um dos sentidos da blocagem é denominado linhas e outro colunas
...
__LS tree species experiment__		
...
__Croqui e anova: comparação entre DIC, DBC e DQL__
...
Ao nível de 5% de significância, há evidências para rejeitarmos $H_0$. Logo, não podemos afirmar que todas as médias de produção para os diferentes tratamentos são iguais.
...
A partir do teste de Tukey, ao nível de 5% de significância, podemos afirmar que a produção média de cana-de-açúcar da variedade C difere das demais produções médias, sendo a da variedade C maior, e as demais produções médias não apresentam diferença significativa. 
...
#' ## Importando os dados
library(readxl)
#dadoscana <- read_xlsx("dados/cana1.1.xlsx")
#knitr::kable(dadoscana)
#str(dadoscana)
#dadoscana <- transform(dadoscana,
 #                  linha = factor(linha),
...
```{r}
#modelocana <- aov(prod ~ linha + coluna + trat,
 #             dadoscana) 
```

```{r}
#' Resíduos Studentizados 
#(res_stud <- rstandard(modelocana))
#ggplot(NULL, 
 #      aes(x=res_stud)) +
#  geom_boxplot() +
